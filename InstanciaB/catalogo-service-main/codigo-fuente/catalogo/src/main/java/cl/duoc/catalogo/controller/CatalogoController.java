 ...
 * @RequestMapping("/api/catalogo") define la URL base para todos los métodos de esta clase.
 */
@RestController
@RequestMapping("/api/catalogo")
public class CatalogoController {

    private final CatalogoService libroService;

    // Inyección de dependencias: el controlador necesita del servicio para funcionar
    public CatalogoController(CatalogoService libroService) {
     ...
     */
    @Operation(summary = "Registrar nuevo catalogo")
    @ApiResponse(responseCode = "201", description = "Catalogo creado exitosamente")    
    @PostMapping
    public Catalogo crearLibro(@RequestBody Catalogo libro) {
     ...
     */
    @Operation(summary = "Listar todos los catalogos",
               description = "Retorna la lista completa de catalogos registrados en el sistema.")
     ...
     */
    @Operation(summary = "Buscar catalogo por ID")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Catalogo encontrado"),
        @ApiResponse(responseCode = "404", description = "Catalogo no encontrado")
    })    
    @GetMapping("/{id}")
    public ResponseEntity<Catalogo> obtenerLibroPorId(
            @Parameter(description = "ID único de catalogo", required = true)
            @PathVariable Long id) {
        Optional<Catalogo> libro = libroService.obtenerPorId(id);
        if (libro.isPresent()) {
     ...
     */
    @Operation(summary = "Actualizar catalogo existente")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Catalogo exitoso"),
        @ApiResponse(responseCode = "404", description = "Catalogo no encontrado")
    })    
    @PutMapping("/{id}")
    public ResponseEntity<Catalogo> actualizarLibro(
            @Parameter(description = "ID del catalogo a actualizar")
            @PathVariable Long id,
            @Valid @RequestBody Catalogo detallesLibro) {
        Catalogo libroActualizado = libroService.actualizarLibro(id, detallesLibro);
     ...
     */
    @Operation(summary = "Eliminar catalogo")
    @ApiResponses({
    ...
    public ResponseEntity<Void> eliminarLibro(
            @Parameter(description = "ID del catalogo a eliminar")
            @PathVariable Long id) {
